...
; (function($, window, document){
		...
		function dealData(){
			...
			loadImg(jsonCache, opts.imgUrlName, function(){
				while(perNum-- > 0 && (data = jsonCache.pop())){
					...
					$wf_img.data({'width':data.width,'height':data.height,'len':$wf_col.children().length,'url':data.image,'title':data.title,'lodUrl':data.url});
					...
					if($('.allFts').length>0){
					    $wf_img.height(84);
						var dataname = ['NTJianZhuan','HanWangT0007','JetLinkBoldHandWritten','minijianxiangli','NaiYou','jin_mei_mxplzx'],dataguid=['80a67cba29f24dbc8379ad52dbc6e1c6','0ccc4adefd4c4f0aa39fc7a3ab51abc2','705d2812313e479eb18a1684df4af118','b193b127656b4f50a0864ba55215e7f1','f185f8aa763f46189fa54adb2dbf0b8c','d00d52c7819141d1bf92d96d111d2a06'];
				        $wf_img.addClass(dataname[idx]);
						imgBigShow.youzikujsm(dataname[idx],dataguid[idx]); 
	...
	var imgBigShow = {
		init : function(o){
			 ...
			 this.count = 0;
			 this.len = o.data('len');
			 this.cssSheet=document.styleSheets[2]
		...
		winrelze : function(){
			...
			$(window).on('resize', function(){
				...
				self.win_view_layer.css({'width':self.wd,'height':self.wh});
			});
		...
		show : function(o){
			 ...
			 this.win_view_layer.children('a').unbind('click')
			 this.win_view_layer.children('a').on('click',function(){
				  self.win_view.hide();
			      self.win_view_layer.hide();
				  if($('.videoSet').length>0){
				     $f().stop();
				  }else if($('.loginWin').length>0){
				      $('.loginWin').hide();
				  }else if($('.uploadWind').length>0){
				      $('.uploadWind').hide();
				  }
			 })
		},
		...
		showFla : function(o){
		    ...
		    o.on('mouseover',function(){
				var l = parseInt($(this).parents('.wf_item').css('left'))+235,t = parseInt($(this).parents('.wf_item').css('top'))+115,url=o.data('url'), htm = '';
				...
				htm+= '<param value="'+url+'" name="movie">'
				...
				htm+= '<embed width="300"  height="300" align="middle" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" wmode="transparent" allowscriptaccess="always" name="undefined" menu="false" quality="high" src="'+url+'">'
				htm+= '</object>'
				htm+='</div>';
		...
		youzikujsm : function(dataname,dataguid){
			...
			var md5 = $.md5(dataguid + elem + resultStr);
			
			var strdatamd5 = md5, strdataguid = dataguid;
			$.getJSON("http://www.youziku.com/webfont/CSSPOST?jsoncallback=?", {"id": md5,"guid": strdataguid,"type": "5"},function(json) {
				if (json.result == 0) {
					/*alert("需要生成");*/
					...
					/*alert("下载css文件");*/
					loadExtentFile("http://www.youziku.com/webfont/css?id=" + md5 + "&guid=" + strdataguid + "&type=5");
				}
			});
		},
